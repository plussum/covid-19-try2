...
if($golist{amt}){
	...
	csv2graph::reduce_cdp_target($amt_country, $AMT_DEF, {geo_type => $REG});
	...
	csv2graph::reduce_cdp_target($amt_pref, $AMT_DEF, {geo_type => $SUBR});
...
if($golist{"tokyo"}){
	...
	my $y2 = {};
	my $marge = {};
	csv2graph::reduce_cdp_target($y1, $TOKYO_DEF, ["positive_count,negative_count"]);
	csv2graph::reduce_cdp_target($y2, $TOKYO_DEF, ["positive_rate"]);
	csv2graph::marge_csv($marge, $y1, $y2);		# Gererate Graph
...
#	severe_case,1,2,3,
	...
	my $y22 = {};
	my $marge2 = {};
	csv2graph::reduce_cdp_target($y21, $TOKYO_ST_DEF, ["hospitalized"]);
...
csv2graph::gen_html_by_gp_list($gp_list, {						# Generate HTML file with graphs
		...
		src_url => "src_url",
		html_file => "$HTML_PATH/csv2graph.html",
		png_path => $PNG_PATH // "png_path",
		png_rel_path => $PNG_REL_PATH // "png_rel_path",
